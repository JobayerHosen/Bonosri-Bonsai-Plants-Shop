...
const ManageOrders = () => {
  ...
  const deleteOrder = (id) => {
    ...
    fetch(`https://bonosri-bonsai.herokuapp.com/orders/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
    ...
    <>
                ...
                {orders.map((order, index) => (
                  <tr key={order?._id}>
                    ...
                    <td>
                      {" "}
                      <span>
                        <img className="order-img" src={order?.productImg} alt="" />
                      </span>
                      {order?.productName}
                    </td>
      ...
      <>
        <Modal show={show} onHide={handleClose} centered>
          ...
          <Modal.Body>
            <Button onClick={() => handleStatusChangle("Shipped")} className="btn-green mx-auto mb-2 d-block">
              Shipped
            </Button>
            <Button onClick={() => handleStatusChangle("Approved")} className="btn-success mx-auto mb-2 d-block">
              Approve
            </Button>
            <Button onClick={() => handleStatusChangle("Pending")} className="btn-primary mx-auto mb-2 d-block">
              Pending
            </Button>
            <Button onClick={() => handleStatusChangle("Rejected")} className="btn-danger mx-auto mb-2 d-block">
              Reject
            </Button>
          </Modal.Body>
          <Modal.Footer>
